    ...
    >
      ...
      >
        <template slot-scope="scope">
          <span>{{ scope.row.beforeStatus | statusFilter(cacheData.statusMap['1012'])}}</span>
        </template>
      </el-table-column>
      <el-table-column
        label="当前状态"
      ...
      >
        <template slot-scope="scope">
          <span>{{ scope.row.status | statusFilter(cacheData.statusMap['1012'])}}</span>
        </template>
      </el-table-column>
      <el-table-column
        label="备注"
...
<script>
import { goodsRecords } from '@/api/goods'
...
export default {
    ...
    methods: {
        ...
        handleQueryList() {
            ...
            goodsRecords(this.$route.params.goodsId)
                .then(response => {
                    this.list = response
                })
                .catch(err => {
