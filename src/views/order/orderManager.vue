  ...
  <div class="app-container">
    <div class="filter-container">
      ...
      <dy-select
        ...
        :selectType="'ORDER_STATUS'"
        :params="externalStatus"
      >
      </dy-select>
      <dy-select
        class="filter-item"
        :placeholder="'内部状态'"
        :selected.sync="listQuery.statusId"
        :selectStyle="'width: 250px;'"
        :selectType="'ORDER_STATUS'"
        :params="innerStatus"
      >
    ...
    >
      ...
      >
        <template slot-scope="scope">
          <span>{{ scope.row.statusId | statusFilter(cacheData.options['ORDER_STATUS'])}}</span>
        </template>
      </el-table-column>
      <!-- <el-table-column
        label="支付方式"
      ...
      >
        <template slot-scope="scope">
          ...
          >核对确认</el-button>
          <el-button
            v-if="scope.row.statusId == 'CONFIRMED'"
            class="action-item"
            type="text"
            v-waves
            @click="formOpen(scope.row, scope.row.statusId)"
...
<script>
import { orderList, orderUpdate, orderUpdateStatus } from '@/api/order'
    ...
    methods: {
        ...
        handleQueryList() {
            ...
            let query = {
                ...
                'params[statusId]': this.listQuery.statusId,
        ...
        handleFormSave() {
            this.$refs['formData'].validate(valid => {
                if (valid) {
                    orderUpdateStatus(this.formData.id, {
